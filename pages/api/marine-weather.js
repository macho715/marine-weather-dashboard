// 해양 기상 데이터 API (Pages Router 버전)
export default function handler(req, res) {
  if (req.method === 'GET') {
    const { lat, lon } = req.query;
    const latitude = parseFloat(lat || '25.0');
    const longitude = parseFloat(lon || '55.0');
    
    const weatherData = generateSampleWeatherData(latitude, longitude);
...
// 샘플 해양 기상 데이터 생성
function generateSampleWeatherData(lat, lon) {
  const data = [];
  const now = new Date();
