...
export async function POST(request: Request) {
  ...
  const body = handler
    ? handler(prompt)
    : buildDefaultInsight(prompt, attachments.length > 0);
...
function buildDefaultInsight(prompt: string, hasAttachments: boolean) {
  if (!hasAttachments) {
    return [
      `요청하신 내용("${prompt}")에 대해 추가 정보가 필요합니다.`,
      "사용 가능한 키워드: weather · risk · schedule",
      "필요하다면 운항 관련 문서를 첨부해 주세요 (PDF/CSV).",
    ].join("\n");
  }

  return [
    `요청하신 내용("${prompt}")을 분석하려면 첨부 파일 형식을 지정해주세요.`,
    "예: 'schedule 분석' 또는 'risk 평가'.",
  ].join("\n");
}
